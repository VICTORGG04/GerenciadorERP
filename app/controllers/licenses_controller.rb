...
post '/licenses' do
  ...
  begin
    ...
    synced = GoogleSheetValidator.register_license!(token, {
      plan:               plan,
      ...
      notes:              params[:notes],
      expires_at:         expires.is_a?(Time) ? expires.strftime('%Y-%m-%d %H:%M:%S') : expires.to_s
    })

    if synced
      flash 'ok', "Licença #{lic.license_ref} cadastrada para #{lic.company_name}!"
    else
      flash 'warn', "Licença #{lic.license_ref} cadastrada para #{lic.company_name}, mas NÃO foi sincronizada com o Google Sheets. Verifique as variáveis de ambiente."
    end
