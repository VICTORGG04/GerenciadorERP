...
if [ -n "$REPO_DIR" ]; then
    ...
    INSTALL_SCRIPT="$REPO_DIR/APK--DEB--BAT(Software)/$PLATFORM/install.sh"

    if [ "$PLATFORM" = "macos" ]; then
        INSTALL_SCRIPT="$REPO_DIR/APK--DEB--BAT(Software)/macos/start.command"
    fi

    if [ "$PLATFORM" = "windows" ]; then
        aviso "No Windows, execute manualmente:"
        echo "  APK--DEB--BAT(Software)\\windows\\setup.bat"
    ...
    if [ -f "$INSTALL_SCRIPT" ]; then
        ...
        echo "Verifique se a plataforma '$PLATFORM' tem install.sh em APK--DEB--BAT(Software)/"
        exit 1
    fi
else
    ...
    REMOTE_PATH="APK--DEB--BAT(Software)/$PLATFORM/install.sh"
    if [ "$PLATFORM" = "macos" ]; then
        REMOTE_PATH="APK--DEB--BAT(Software)/macos/start.command"
    fi

    if [ "$PLATFORM" = "windows" ]; then
        aviso "No Windows, visite o repositório e baixe:"
        echo "  https://github.com/VICTORGG04/GerenciadorClaude"
        echo "  Execute APK--DEB--BAT(Software)\\windows\\setup.bat como Administrador"
        exit 0
    fi
